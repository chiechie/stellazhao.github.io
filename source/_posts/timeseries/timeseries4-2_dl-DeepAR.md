...
- 适用于噪声较大的数据：相较于点估计能提供更有用的信息，如方差，在金融领域这个值代表风险，价值的(比如var用于计算准备金)。在交易上的启发：构建基于波动率的策略，风险大的情况下不交易。

## DeepAR原理


- 怎么得到条件概率分布，或后验概率，即p（z｜x），训练的时候分两步走
- 第一步：先确定概率分布的函数族，比如高斯分布，那么使用参数< $\mu, \sigma$> 来定义这个分布。
- 第二步：估计这个分布的参数< $\mu, \sigma$​>,  使用一个神经网络来拟合这个概率分布，具体来说
  ...
  - 使用最大似然估计的方法来训练这个神经网络：$\mathcal{L}=-\sum\limits_{i=1}^{N} \sum\limits_{t=t_{0}}^{T} \log \ell\left(z_{i, t} \mid \theta\left(\mathbf{h}_{i, t}\right)\right)$
      > 模型输出的分布参数为<$\mu, \sigma$> , ground truth为y，loss为likelihood取negtive log：$-log Prob(\mu,\sigma,y)$​， -log Prob就是反馈信号，取值越大意味着当前参数不好，还要调整
- 做推断的时候，输入x，得到后验概率p（z｜x）对应的分布<$\mu, \sigma$​>，在分布中进行采样，得到预测值z
...
1. 对一个神经网络喂入不同曲线，可能让模型学习到 或者 匹配到 混乱模式。但是，即使学到了 混合的模式，未必是件坏事，要分两个层面看：
   1. 混合是个好事：比如冷启动的商品或者缺失值过多的商品，如果能借鉴 模式相近（学出来的）的数据质量很好的其他商品 ，比仅从自己的历史数据学习效果更好。
   2. 混合是个坏事：比如数据质量本身就很好，不用参考别人的学习资料（可能还会拖后腿）

1. 如果希望用一个global 模型一劳永逸对所有商品建模，至少要考虑到 ，单个商品的 local模型 和 global 模型 要择优录取。

2. 如何让模型区分类型接近的商品要和类型不同的商品，从而做出不一样的预测？将商品的属性信息进行编码：

   - 方法1: 每个商品使用id做embedding，知识只能通过id迁移，只能对id出现在训练阶段的item做预测。

   - 方法2: 每个商品使用属性做embedding，知识可以通过商品属性迁移，可以对id没有出现在训练阶段的item做预测。
