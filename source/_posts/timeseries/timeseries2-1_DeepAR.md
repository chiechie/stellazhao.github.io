...
- 经典时序预测： arima/state space/exponential smoothing
...
- 我们希望得到条件概率分布，或后验概率，即p（z｜x），分两步走
...
- 第二步：估计这个分布的参数< $\mu, \sigma$​>,  使用一个神经网络来拟合这个概率分布，具体来说
  ...
  - 使用最大似然估计的方法来训练这个神经网络：
      - 模型输出的分布参数为<$\mu, \sigma$​> , ground truth为y，loss为负的对数likelihood：$-log Prob(\mu,\sigma,y)$​
- 使用likelihood 和 y 来度量输出的<$\mu, \sigma$> 质量好不好。有点像，有个人号称 他的理论知识渊博<$\mu, \sigma$​> (覆盖了y空间中所有的案例），现在要检验他的方法论对不对，就拿参数验证下，能否完美解释观测数据。
- 总结下：
